...
- macOS 或 Windows。
- Chrome 141 或更新版本（原子关闭 Side Panel 需要），建议记录完整版本号。
- Node.js 22 或更高版本。
- pnpm 10.15.0。
- 一个可以正常使用的 Fomo 账号。
...
3. 保持一个已登录的 Fomo 测试标签页打开，然后打开侧边栏或悬浮窗。
4. 确认插件自动刷新至多一个 Fomo 标签页并恢复连接，无须手动刷新页面。

MAIN-world observer 只能在页面注入后观测新建的 WebSocket，因此插件重新加载后
会先 ping 当前内容脚本；只有确认页面仍运行旧脚本时，才会限次自动刷新最近使用的
一个 Fomo 标签页。自动恢复失败时仍保留本地历史只读能力。
...
悬浮窗和侧边栏是同一信息流的两种展示方式。它们共享后台连接、本地历史、筛选、设置和备注；切换时先同步目标界面，再自动关闭原界面，任何时刻只保留一个插件界面。悬浮窗可自由拖动和调整大小，尺寸会被记住并在下次打开时恢复。
...
1. 在侧边栏设置中点击“悬浮窗”，保持已登录的 Fomo 标签页打开。
2. 确认出现“正在切换…”，随后打开一个独立小窗。
3. 确认原侧边栏自动关闭，悬浮窗立即显示此前的历史、筛选和设置。
4. 在悬浮窗设置中点击“侧边栏”，确认侧边栏同步完成后悬浮窗自动关闭。
5. 两次切换期间分别等待一条新动态，确认没有遗漏或重复。
...
3. 从设置切回侧边栏，确认同一份历史与未读状态一致；筛选、交易员备注、置顶/静音保持同步，悬浮窗自动关闭。

### MT-19：service worker 重启后的恢复

1. 打开悬浮窗。
2. 在 `chrome://extensions` 中对该扩展点“重新加载”（或等待 service worker 被系统挂起后再唤醒）。
3. 不手动刷新 Fomo 页面，确认插件自动刷新至多一个已有 Fomo 标签页并恢复“已连接”。
4. 再次点击扩展图标，确认聚焦到已存在的悬浮窗，而不是新开一个。
5. 退出 Fomo 登录后重复操作，确认只提示登录，不产生循环刷新。
